...
?>
	...
	if ( ! empty( $data_columns ) ) {
		...
		foreach ( $data_columns as $column ) {
			...
			$background_image = isset( $column['background-image'] ) && ! empty( $column['background-image'] );
			$background_attribute = ( $background_image ) ? ' background="' . esc_url( $column['background-image'] ) . '"': '';
